...
void initialize(sfmt_t *rng, od_t *od) {
    ...
    *od = ( od_t ) {
        .w_aff = calloc(N_PRE * N_POST, sizeof(float)),
    ...
    for (int32_t i = 0; i < N_POST; i++) {
        od -> v[i] = V_INIT + 10.0 * sfmt_genrand_real2(rng);
    }
    ...
    for (int32_t i = 0; i < N_POST; i++) {
        for (int32_t j = 0; j < N_POST; j++) {
            ...
            od -> w_lat[j + N_POST * i] = (
                norm_coef * (K1 * exp(-d2 / (2.0 * S1)) - K2 * exp(-d2 / (2.0 * S2)))
            );
        }
    }
}
...
void calculateSynapticInputs(sfmt_t *rng, od_t *od) {
    ...
    for (int32_t i = 0; i < N_POST; i++) {
        float r = 0.0;
        for (int32_t j = 0; j < N_PRE; j++) {
            r += od -> w_aff[j + N_PRE * i] * od -> spike_pre[j];
        }
    ...
    for (int32_t i = 0; i < N_POST; i++) {
        float r = 0.0;
        for (int32_t j = 0; j < N_POST; j++) {
            r += od -> w_lat[j + N_POST * i] * od -> spike_post[j];
        }
...
void updateSynapticWeights(od_t *od) {
    ...
    for (int32_t i = 0; i < N_POST; i++) {
        for (int32_t j = 0; j < N_PRE; j++) {
            int32_t k = j + N_PRE * i;
            od -> w_aff[k] += (
                -A_d * od -> spike_pre[j] * od -> trace_post[i] +
                A_p * od -> trace_pre[j] * od -> spike_post[i]
            );
            od -> w_aff[k] = (od -> w_aff[k] < 0.0) ? 0.0 : od -> w_aff[k];
        }
    }
}
