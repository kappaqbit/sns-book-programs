...
static inline double alpha_n(const double v) {
    return (0.1 - 0.01 * (v - E_REST)) / (exp(1.0 - 0.1 * (v - E_REST)) - 1.0);
}
...
//定常状態でのm, h, n
static inline double m0(const double v) {
    return alpha_m(v) / (alpha_m(v) + beta_m(v));
}

static inline double h0(const double v) {
    return alpha_h(v) / (alpha_h(v) + beta_h(v));
}

static inline double n0(const double v) {
    return alpha_n(v) / (alpha_n(v) + beta_n(v));
}

// ゲート変数それぞれの時定数
static inline double tau_m(const double v) {
    return 1.0 / (alpha_m(v) + beta_m(v));
}

static inline double tau_h(const double v) {
    return 1.0 / (alpha_h(v) + beta_h(v));
}

static inline double tau_n(const double v) {
    return 1.0 / (alpha_n(v) + beta_n(v));
}

// ゲート変数
static inline double dmdt(const double v, const double m) {
    return (1.0 / tau_m(v)) * (-m + m0(v));
}

static inline double dhdt(const double v, const double h) {
    return (1.0 / tau_h(v)) * (-h + h0(v));
}

static inline double dndt(const double v, const double m) {
    return (1.0 / tau_n(v)) * (-m + n0(v));
}
...
int main(void) {
    ...
    for (int32_t nt = 0; nt < NT; nt++) {
        ...
        double dmdt3 = dmdt(v + 0.5 * DT * dmdt2, m + 0.5 * DT * dmdt2);
        double dhdt3 = dhdt(v + 0.5 * DT * dhdt2, h + 0.5 * DT * dhdt2);
        double dndt3 = dndt(v + 0.5 * DT * dndt2, n + 0.5 * DT * dndt2);
        double dvdt3 = dvdt(
            v + 0.5 * DT * dvdt2,
            m + 0.5 * DT * dmdt2,
            h + 0.5 * DT * dhdt2,
        ...
        double dmdt4 = dmdt(v + DT * dmdt3, m + DT * dmdt3);
        double dhdt4 = dhdt(v + DT * dhdt3, h + DT * dhdt3);
        double dndt4 = dndt(v + DT * dndt3, n + DT * dndt3);
        double dvdt4 = dvdt(
            v + DT * dvdt3,
            m + DT * dmdt3,
            h + DT * dhdt3,
