// plot 'multi.dat' using 1:10 with lines
// plot [1000:1050] 'multi.dat' using 1:9 w l, 'multi.dat' using 1:12 w l, 'multi.dat' using 1:15 w l, 'multi.dat' using 1:18 w l
...
static inline double pir2(const double rad) {
    return M_PI * rad * rad;
}
...
const double phi[NC] = { 7769.0, 7769.0, 7769.0, 7769.0, 7769.0, 7769.0, 7769.0, 34530, 17402.0, 26404.0, 5941.0, 5941.0, 5941.0, 5941.0, 5941.0, 5941.0, 5941.0, 5941.0, 5941.0 };
...
static inline double alpha_m(const double v) {
    return (0.32 * (13.1 - (v - V_LEAK))) / (exp((13.1 - (v - V_LEAK)) / 4.0) - 1.0);
}

static inline double alpha_s(const double v) {
    return 1.6 / (1 + exp(-0.072 * ((v - V_LEAK) - 65.0)));
}

static inline double alpha_n(const double v) {
    return (0.016 * (35.1 - (v - V_LEAK))) / (exp((35.1 - (v - V_LEAK)) / 5.0) - 1);
}

static inline double alpha_c(const double v) {
    return (v <= (50 + V_LEAK)) ? exp(((v - V_LEAK) - 10.0) / 11.0 - ((v - V_LEAK) - 6.5) / 27.0) / 18.975 : 2.0 * exp(-((v - V_LEAK) - 6.5) / 27.0);
}

static inline double alpha_a(const double v) {
    return (0.02 * (13.1 - (v - V_LEAK))) / (exp((13.1 - (v - V_LEAK)) / 10) - 1.0);
}

static inline double alpha_h(const double v) {
    return 0.128 * exp((17.0 - (v - V_LEAK)) / 18.0);
}

static inline double alpha_r(const double v) {
    return (v <= (0 + V_LEAK)) ? 0.005 : exp(-(v - V_LEAK) / 20.0) / 200.0;
}

static inline double alpha_b(const double v) {
    return 0.0016 * exp((-13.0 - (v - V_LEAK)) / 18.0);
}

static inline double alpha_q(const double x) {
    return fmin(0.2e-4 * x, 0.01);
}

static inline double beta_m(const double v) {
    return (0.28 * ((v - V_LEAK) - 40.1)) / (exp(((v - V_LEAK) - 40.1) / 5.0) - 1);
}

static inline double beta_s(const double v) {
    return (0.02 * ((v - V_LEAK) - 51.1)) / (exp(((v - V_LEAK) - 51.1) / 5.0) - 1.0);
}

static inline double beta_n(const double v) {
    return 0.25 * exp((20.0 - (v - V_LEAK)) / 40.0);
}

static inline double beta_c(const double v) {
    return (v <= (50 + V_LEAK)) ? 2.0 * exp(-((v - V_LEAK) - 6.5) / 27.0) - alpha_c(v) : 0;
}

static inline double beta_a(const double v) {
    return (0.0175 * ((v - V_LEAK) - 40.1)) / (exp(((v - V_LEAK) - 40.1) / 10.0) - 1);
}

static inline double beta_h(const double v) {
    return 4.0 / (1 + exp((40.0 - (v - V_LEAK)) / 5.0));
}

static inline double beta_r(const double v) {
    return (v <= (0 + V_LEAK)) ? 0 : 0.005 - alpha_r(v);
}

static inline double beta_b(const double v) {
    return 0.05 / (1 + exp((10.1 - (v - V_LEAK)) / 5.0));
}

static inline double beta_q(const double x) {
    return 0.001;
}
...
void initialize(double var[N_VARS][NC], double I_inj[NC], double g_comp[NC][2]) {
    ...
    for (int32_t i = 0; i < NC; i ++) {
        g_comp[i][0] = (i == 0) ? 0.0 : 2.0 / ((Ri * len[i - 1]) / pir2(rad[i - 1]) + (Ri * len[i]) / pir2(rad[i]));
        g_comp[i][1] = (i == NC - 1) ? 0.0 : 2.0 / ((Ri * len[i]) / pir2(rad[i]) + (Ri * len[i + 1]) / pir2(rad[i + 1]));
    }
}
...
void solve_euler(double var[N_VARS][NC], double I_inj[NC], double g_comp[NC][2]) {
    ...
    for (int32_t i = 0; i < NC; i++) {
        double *v = var[V];
        for (int32_t j = 2; j < N_VARS - 1; j++) {
            dvar[j][i] = (DT / tau(j, v[i])) * (inf(j, v[i]) - var[j][i]);
        }
        dvar[Q][i] = (DT / tau(Q, var[XI][i])) * (inf(Q, var[XI][i]) - var[Q][i]);
        ...
        I_comp[i] = (
            ((i == 0) ? 0.0 : g_comp[i][0] * (v[i - 1] - v[i]) / area[i]) +
            ((i == NC - 1) ? 0.0 : g_comp[i][1] * (v[i + 1] - v[i]) / area[i])
        );

        dvar[V][i] = (DT / Cm) * (-I_ion[i] + I_comp[i] + I_inj[i]);

        double I_Ca = g_Ca[i] * area[i] * var[S][i] * var[S][i] * var[R][i] * (v[i] - V_Ca);
...
int main(void) {
    ...
    for (int32_t nt = 0; nt < NT; nt++) {
        ...
        printf("%f ", t);
        for (int32_t i = 0; i < NC; i++) {
            printf("%f%s", var[V][i], (i == NC - 1) ? "\n" : " ");
        }

        //I_inj[8] = 1.0e-4 / area[8];
        I_inj[8] = (1000 <= t && t < 1001) ? 1.0e-2 / area[8] : 0.;
